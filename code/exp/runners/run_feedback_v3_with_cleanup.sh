...
RUN_ROOT="${1:-/disk/chojm/experiments/rlm_saga_v1_feedback_v3_live_$(date +%Y%m%d_%H%M%S)}"
mkdir -p \
  ...
  "$RUN_ROOT/reports"
echo "$RUN_ROOT" > /tmp/rlm_saga_feedback_v3_run_root.txt
...
if ! curl -s http://localhost:8000/v1/models >/dev/null 2>&1; then
  echo "[feedback_v3] vLLM failed to start" >&2
  exit 1
fi
...
run_stage() {
  ...
  python -m exp.runners.run_paired \
    --config exp/config/experiment_feedback_v3.yaml \
    --stage "$STAGE" \
    --variants V0 V3 \
    --run-root "$RUN_ROOT" \
    --baseline-out "$BASE" \
...
run_stage stage_feedback_v3_main main
MAIN_EXIT=$?

run_stage stage_feedback_v3_recovery recovery
RECOVERY_EXIT=$?
