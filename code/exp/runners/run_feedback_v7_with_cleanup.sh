...
RUN_ROOT="${1:-/disk/chojm/experiments/rlm_saga_v1_feedback_v7_$(date +%Y%m%d_%H%M%S)}"
mkdir -p \
...
curl -s http://localhost:8000/v1/models >/dev/null 2>&1 || {
  echo "[feedback_v7] vLLM failed to start" >&2
  exit 1
}
...
run_stage() {
  ...
  python -m exp.runners.run_paired \
    --config exp/config/experiment_feedback_v7.yaml \
    --stage "$STAGE" \
    --variants "${VARIANTS[@]}" \
    --run-root "$RUN_ROOT" \
    --baseline-out "$BASE" \
