...
RUN_ROOT="${1:-/disk/chojm/experiments/rlm_saga_v1_feedback_v6_$(date +%Y%m%d_%H%M%S)}"
mkdir -p \
  ...
  "$RUN_ROOT/reports"
echo "$RUN_ROOT" > /tmp/rlm_saga_feedback_v6_run_root.txt
...
curl -s http://localhost:8000/v1/models >/dev/null 2>&1 || {
  echo "[feedback_v6] vLLM failed to start" >&2
  exit 1
}
...
run_stage() {
  ...
  python -m exp.runners.run_paired \
    --config exp/config/experiment_feedback_v6.yaml \
    --stage "$STAGE" \
    --variants V0 V3_BASE V3_PREFIX \
    --run-root "$RUN_ROOT" \
    --baseline-out "$BASE" \
    --extension-out "$EXT" \
  ...
  python -m exp.analysis.summarize \
    ...
    --report-out "$RUN_ROOT/reports/rlm_vs_v6_${TAG}_runtime.md" \
...
RUN_EXIT=0
run_stage stage_feedback_v6_smoke smoke || RUN_EXIT=$?
if [[ "$RUN_EXIT" -eq 0 ]]; then
  run_stage stage_feedback_v6_main main || RUN_EXIT=$?
  run_stage stage_feedback_v6_recovery recovery || RUN_EXIT=$?
  run_stage stage_feedback_v6_boundary boundary || RUN_EXIT=$?
fi

# P9 recovery loss-case analysis: where V0 succeeds but V3_PREFIX fails.
python -m exp.analysis.analyze_loss_cases \
  --input-jsonl "$RUN_ROOT/results/summary/recovery_dual_eval/rescored_runtime.jsonl" \
  --left V0 \
  --right V3_PREFIX \
  --stage stage_feedback_v6_recovery \
  --problem-prefix P9 \
