...
RUN_ROOT="${1:-/disk/chojm/experiments/rlm_saga_v1_recommended_v0_v2_s012_$(date +%Y%m%d_%H%M%S)}"
...
echo "$RUN_ROOT" > /tmp/rlm_saga_recommended_v0_v2_s012_run_root.txt
...
if ! curl -s http://localhost:8000/v1/models >/dev/null 2>&1; then
  echo "[recommended] vLLM failed to start" >&2
  exit 1
fi

set +e
python -m exp.runners.run_paired \
  --config exp/config/experiment_recommended_v0_v2_s012.yaml \
  --stage stage_recommended_v0_v2_s012 \
  --variants V0 V2 \
  --run-root "$RUN_ROOT" \
