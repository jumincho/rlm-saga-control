...
RUN_ROOT="${1:-/disk/chojm/experiments/rlm_saga_v1_feedback_v7_6_full_$(date +%Y%m%d_%H%M%S)}"
CONFIG_PATH="${2:-exp/config/experiment_feedback_v7_6.yaml}"
...
LATEST_V75="$(ls -dt /disk/chojm/experiments/rlm_saga_v1_feedback_v7_5_full_* 2>/dev/null | head -n 1 || true)"
if [[ -n "$LATEST_V75" ]]; then
  python -m exp.analysis.analyze_immutability_taxonomy \
    --baseline "$LATEST_V75/results/raw/baseline_boundary_full.jsonl" \
    --extension "$LATEST_V75/results/raw/extension_boundary_full.jsonl" \
    --variant V3_PREFIX_SPLIT \
    --out-md "$RUN_ROOT/reports/immutability_failure_taxonomy_v7_5.md" \
    --out-csv "$RUN_ROOT/results/summary/immutability_failure_taxonomy_v7_5.csv" \
    2>&1 | tee "$RUN_ROOT/runs/runner_logs/immutability_taxonomy_v7_5.log"
fi
...
run_stage "stage_feedback_v7_6_smoke" "v7_6_smoke"
run_stage "stage_feedback_v7_6_debug" "v7_6_debug"
run_stage "stage_feedback_v7_6_boundary_dev" "v7_6_boundary_dev"
run_stage "stage_feedback_v7_6_boundary_test" "v7_6_boundary_test"
