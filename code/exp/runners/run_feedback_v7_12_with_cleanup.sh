...
RUN_ROOT="${1:-/disk/chojm/experiments/rlm_saga_v1_feedback_v7_12_full_$(date +%Y%m%d_%H%M%S)}"
CONFIG_PATH="${2:-exp/config/experiment_feedback_v7_12.yaml}"
...
run_stage() {
  ...
  python -m exp.analysis.gate_alignment_report \
    --runtime-jsonl "$EXT" \
    --strict-jsonl "$SUM_DIR/dual_eval/rescored_strict.jsonl" \
    --variant V3_PREFIX_SPLIT \
    --out-md "$RUN_ROOT/reports/gate_alignment_${TAG}.md" \
    --out-csv "$SUM_DIR/gate_alignment_${TAG}.csv" \
    --max-cases 10 \
    2>&1 | tee "$RUN_ROOT/runs/runner_logs/gate_alignment_${TAG}.log"
  ...
  python -m exp.analysis.state_timeline_failure_breakdown \
    --input-jsonl "$SUM_DIR/dual_eval/rescored_strict.jsonl" \
    --variant V3_PREFIX_SPLIT \
    --out-md "$RUN_ROOT/reports/state_timeline_failure_breakdown_${TAG}.md" \
    --max-cases 40 \
    2>&1 | tee "$RUN_ROOT/runs/runner_logs/state_timeline_failure_breakdown_${TAG}.log"
...
run_stage "stage_feedback_v7_12_smoke" "v7_12_smoke"
run_stage "stage_feedback_v7_12_debug" "v7_12_debug"
run_stage "stage_feedback_v7_12_boundary_dev" "v7_12_boundary_dev"
run_stage "stage_feedback_v7_12_boundary_test" "v7_12_boundary_test"
run_stage "stage_feedback_v7_12_boundary_test_v2" "v7_12_boundary_test_v2"
run_stage "stage_feedback_v7_12_boundary_test_v3" "v7_12_boundary_test_v3"
run_stage "stage_feedback_v7_12_boundary_test_v4" "v7_12_boundary_test_v4"
run_stage "stage_feedback_v7_12_boundary_test_v5" "v7_12_boundary_test_v5"
run_stage "stage_feedback_v7_12_boundary_test_v6" "v7_12_boundary_test_v6"
run_stage "stage_feedback_v7_12_boundary_test_v7" "v7_12_boundary_test_v7"
