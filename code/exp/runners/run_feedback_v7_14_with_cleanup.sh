...
RUN_ROOT="${1:-/disk/chojm/experiments/rlm_saga_v1_feedback_v7_14_full_$(date +%Y%m%d_%H%M%S)}"
CONFIG_PATH="${2:-exp/config/experiment_feedback_v7_14.yaml}"
...
run_stage() {
  ...
  python -m exp.analysis.gate_fp_breakdown \
    --runtime-jsonl "$EXT" \
    --strict-jsonl "$SUM_DIR/dual_eval/rescored_strict.jsonl" \
    --variant V3_PREFIX_SPLIT \
    --out-md "$RUN_ROOT/reports/gate_fp_breakdown_${TAG}.md" \
    --out-csv "$SUM_DIR/gate_fp_breakdown_${TAG}.csv" \
    --max-cases 10 \
    --top-k 20 \
    2>&1 | tee "$RUN_ROOT/runs/runner_logs/gate_fp_breakdown_${TAG}.log"
...
run_stage "stage_feedback_v7_14_smoke" "v7_14_smoke"
run_stage "stage_feedback_v7_14_debug" "v7_14_debug"
run_stage "stage_feedback_v7_14_boundary_dev" "v7_14_boundary_dev"
run_stage "stage_feedback_v7_14_boundary_test_v9" "v7_14_boundary_test_v9"
