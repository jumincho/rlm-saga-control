...
RUN_ROOT="${1:-/disk/chojm/experiments/rlm_saga_v1_feedback_v7_3_$(date +%Y%m%d_%H%M%S)}"
MODE="${2:-full}" # full | boundary_only
CONFIG_PATH="${V73_CONFIG_PATH:-exp/config/experiment_feedback_v7_3.yaml}"
BOUNDARY_STAGE="${V73_BOUNDARY_STAGE:-stage_feedback_v7_3_boundary_full}"
BOUNDARY_VARIANTS_STR="${V73_BOUNDARY_VARIANTS:-V0 V0_SPLIT_ONLY V3_PREFIX_SPLIT V3_PREFIX_NO_SPLIT}"
MAIN_STAGE="${V73_MAIN_STAGE:-stage_feedback_v7_3_main_regression}"
RECOVERY_STAGE="${V73_RECOVERY_STAGE:-stage_feedback_v7_3_recovery_regression}"
...
curl -s http://localhost:8000/v1/models >/dev/null 2>&1 || {
  echo "[feedback_v7_3] vLLM failed to start" >&2
  exit 1
}
...
run_stage() {
  ...
  python -m exp.analysis.summarize \
    ...
    --report-out "$RUN_ROOT/reports/rlm_vs_v7_3_${TAG}_runtime.md" \
...
if [[ "$RUN_EXIT" -eq 0 && "$MODE" != "boundary_only" && "${V73_SKIP_REGRESSION:-0}" != "1" ]]; then
...
for T in boundary_full main_regression recovery_regression; do
  if [[ -f "$RUN_ROOT/results/summary/${T}_dual_eval/rescored_runtime.jsonl" ]]; then
    if [[ "$T" == "boundary_full" ]]; then
      for RIGHT in V0_SPLIT_ONLY V3_PREFIX_SPLIT V3_PREFIX_NO_SPLIT; do
        python -m exp.analysis.paired_stats \
          ...
          2>&1 | tee "$RUN_ROOT/runs/runner_logs/paired_stats_${T}_${RIGHT,,}.log" || true
      done
    else
      python -m exp.analysis.paired_stats \
        --input-jsonl "$RUN_ROOT/results/summary/${T}_dual_eval/rescored_runtime.jsonl" \
        --left V0 \
        --right V3_PREFIX \
        --out-json "$RUN_ROOT/results/summary/paired_stats_${T}_runtime_v0_vs_v3_prefix.json" \
